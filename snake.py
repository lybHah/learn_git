...
class Grid(object):
	def __init__(self,master=None,window_width=800,window_height=600,grid_width=50,offset=10):
		...
		self.bg="#EBEBEB"
		self.canvas=Canvas(master,width+2*self.offset,height=self.height+2*self.offset,bg=self.bg)
		self.canvas.pack()
		self.grid_list()
...
class Snake(object):
	...
	def move(self):
		...
		if not new in self.availbale_grid():
			self.status.reverse()
			self.gameover=True
		else:
...
class SnakeGame(Frame):
	...
	def run(self):
		...
		if self.snake.gameover==True:
			message=tkMessageBox.showinfo("Sb!","your score is: %d" %self.snake.score)
			if message=='ok':
				sys.exit()
